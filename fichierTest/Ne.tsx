  ...
  const initGoogleMaps = useCallback(() => {
    if (typeof window !== "undefined" && window.google?.maps?.places) {
      try {
        ...
        if (mapRef.current) {
          // Centre par défaut sur Montréal
        ...
        if (status === window.google.maps.places.PlacesServiceStatus.OK && place) {
          ...
          if (place.geometry?.location && mapRef.current) {
            ...
            window.currentMarker = new window.google.maps.Marker({
              ...
              map: map,
              title: place.name || suggestion.description,
        ...
        <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700">
          ...
          <div className="p-4 sm:p-6 border-b border-gray-200 dark:border-gray-700">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {/* Colonne de gauche : recherche d'adresse */}
                ...
                <div className="relative" ref={suggestionsRef}>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder={googleLoaded ? "Tapez une adresse..." : "Entrez l'adresse"}
              ...
              <div className="lg:block">
                ...
                <div 
                  ref={mapRef} 
                  className="w-full h-[300px] rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-900"
                />
                {!googleLoaded && GOOGLE_MAPS_API_KEY && (
                  <div className="flex items-center justify-center h-[300px]">
                    <Loader2 size={30} className="animate-spin text-[var(--color-primary)]" />
                  </div>
                )}
                {!GOOGLE_MAPS_API_KEY && (
                  <div className="flex items-center justify-center h-[300px] bg-gray-100 dark:bg-gray-900 rounded-xl">
          ...
          <div className="p-4 sm:p-6 border-b border-gray-200 dark:border-gray-700">
            ...
            <textarea value={formData.commentaires} onChange={(e) => setFormData({ ...formData, commentaires: e.target.value })} placeholder="Notes..." rows={3} className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white resize-none" />
          </div>
          ...
          <div className="p-4 sm:p-6 flex flex-col sm:flex-row gap-3">
            ...
            <button type="submit" disabled={loading} className="flex-1 flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl font-semibold text-white disabled:opacity-50" style={{ background: "linear-gradient(135deg, var(--color-primary), var(--color-primary-dark))" }}>
...
function Toggle({ label, checked, onChange }: { label: string; checked: boolean; onChange: (c: boolean) => void }) {
  return (
    <button type="button" onClick={() => onChange(!checked)} className={`flex items-center gap-3 px-4 py-3 rounded-xl border ${checked ? "bg-[var(--color-primary)]/10 border-[var(--color-primary)]/30 text-[var(--color-primary)]" : "bg-gray-50 dark:bg-gray-900 border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400"}`}>
